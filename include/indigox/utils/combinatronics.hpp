...
template <class T, class ICT = std::vector<T>>
  ...
  bool operator()(innerType& c) {
    ...
    for (auto it = iters.begin(); ; ) {
      ...
      if (it->third == it->second) {
        if (it + 1 == iters.end()) finished = true;
        else {
          it->third = it->first;
          ++it;
        }
...
struct RegionalPermutation {
  ...
  RegionalPermutation(RandomIt begin, RandomIt end) : b(begin), e(end),
  finished(false), first_pass(false) { }
