...
namespace indigox::graph {
  ...
  public:
    ...
    inline uint32_t GetPreAssignedCount() const {
      return _pre;
    }
    ...
    inline void SetPreAssignedCount(const uint32_t count) {
      _pre = count;
    }
    ...
    inline uint32_t GetAssignedCount() const {
      return _count;
    }
    ...
    inline uint32_t GetTotalAssigned() const {
      return _pre + _count;
    }

    /*! \brief Set the number of assigned eletrons.
     *  \param count the number of electrons to assign. */
    inline void SetAssignedCount(const uint32_t count) {
      _count = count;
    }
  ...
  public:
    ...
    inline size_t Degree(const AGVertex &v) const {
      return HasVertex(v) ? _g.Degree(v.get())
    ...
    inline std::pair<NbrsIter, NbrsIter>
    GetNeighbours(const AGVertex &v) const {
