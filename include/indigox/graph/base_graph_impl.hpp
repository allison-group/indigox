...
BASEGRAPH(struct)::BaseImpl {
  ...
  BaseImpl() : boost_graph(), state(0), state_cached_components(0), state_cached_cycles(0) { }
  ...
  void AddEdge(const V& u, const V& v, const E& e) {
    ...
    EdgeType eboost = boost::add_edge(uboost, vboost, EP(), boost_graph).first;
  ...
  void load(Archive& archive, const uint32_t) {
    ...
    for (size_t pos = 0; pos < input_edges.size(); ++pos) {
      AddEdge(input_vertex_edges[pos].first, input_vertex_edges[pos].second, input_edges[pos]);
      }
...
//  }
  ...
  BASEGRAPH(void)::RemoveVertex(const V &v) {
    ...
    PredIter vi, vi_end;
    std::tie(vi, vi_end) = boost::inv_adjacent_vertices(vboost, m_basedata->boost_graph);
    for (; vi != vi_end; ++vi) {
      V u = GetV(*vi);
      E e = GetE(boost::edge(vboost, *vi, m_basedata->boost_graph).first);
      m_basedata->edge_descriptors.erase(e);
      m_basedata->edges.erase(std::find(m_basedata->edges.begin(), m_basedata->edges.end(), e));
    }
    // Remove incident edges of directed graphs
    if (D::is_directed) {
      NbrsIter vp, vp_end;
      std::tie(vp, vp_end) = boost::adjacent_vertices(vboost, m_basedata->boost_graph);
      for (; vp != vp_end; ++vp) {
        V u = GetV(*vp);
    ...
    m_basedata->vertices.erase(std::find(m_basedata->vertices.begin(), m_basedata->vertices.end(), v));
  ...
  BASEGRAPH(bool)::HasVertex(const V &v) const {
    return m_basedata->vertex_descriptors.left.find(v) != m_basedata->vertex_descriptors.left.end();
  }

  BASEGRAPH(bool)::HasEdge(const E &e) const {
    return m_basedata->edge_descriptors.left.find(e) != m_basedata->edge_descriptors.left.end();
  }
  ...
  BASEGRAPH(E)::GetEdge(const V &u, const V &v) const {
    ...
    VertType vboost = GetDescriptor(v);
    EdgeType eboost = boost::edge(uboost, vboost, m_basedata->boost_graph).first;
    return GetE(eboost);
  }
  ...
  BASEGRAPH(const tBG::ComponentContain &)::GetConnectedComponents() {
    ...
    algorithm::ConnectedComponents(*this, m_basedata->cached_connected_components);
  ...
  BASEGRAPH(bool)::IsCyclic(const V &v) {
    GetCycles();
    auto pos = std::find(m_basedata->cached_cyclic_vertices.begin(), m_basedata->cached_cyclic_vertices.end(), v);
  ...
  BASEGRAPH(bool)::IsCyclic(const E &e) {
    GetCycles();
    auto pos = std::find(m_basedata->cached_cyclic_edges.begin(), m_basedata->cached_cyclic_edges.end(), e);
