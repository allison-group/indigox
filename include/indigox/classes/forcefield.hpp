...
namespace indigox {
  ...
  public:
    INDIGOX_GENERIC_PIMPL_CLASS_DEFAULTS(FFAtom);
    INDIGOX_GENERIC_PIMPL_CLASS_OPERATORS(FFAtom, atm);
  ...
  public:
     ...
     *  \details Though other values are provided, currently only the harmonic
     * and quartic function types are technically supported. */
    enum class Type {
      Empty,    //!< An empty bond type with no parameters
      Harmonic, //!< A harmonic bond type taking the form \f$\frac{k_b}{2}(x -
                //!< x_0)^2\f$.
      Quartic,  //!< A quartic bond type taking the form \f$\frac{k_b}{2}(x^2 -
                //!< {x_0}^2)^2\f$.
      Morse,
      Cubic,
      FENE
  ...
  public:
    INDIGOX_GENERIC_PIMPL_CLASS_DEFAULTS(FFBond);
    INDIGOX_GENERIC_PIMPL_CLASS_OPERATORS(FFBond, bnd);
  ...
  public:
     ...
     *  \details Though other values are provided, currently only the harmonic
     * and cosine-harmonic function types are technically supported. */
    enum class Type { Empty, Harmonic, CosineHarmonic, UreyBradley, Quartic };
  ...
  public:
    INDIGOX_GENERIC_PIMPL_CLASS_DEFAULTS(FFAngle);
    INDIGOX_GENERIC_PIMPL_CLASS_OPERATORS(FFAngle, ang);
  ...
  public:
    INDIGOX_GENERIC_PIMPL_CLASS_DEFAULTS(FFDihedral);
    INDIGOX_GENERIC_PIMPL_CLASS_OPERATORS(FFDihedral, dhd);
  ...
  public:
    ...
    enum class Family { Empty, GROMOS, CHARMM, AMBER, Other };
  ...
  public:
    INDIGOX_GENERIC_PIMPL_CLASS_DEFAULTS(Forcefield);
    INDIGOX_GENERIC_PIMPL_CLASS_OPERATORS(Forcefield, ff);
