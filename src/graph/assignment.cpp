...
namespace indigox::graph {
  IXAssignmentGraph::IXAssignmentGraph(MolecularGraph g) : _source(g), _g() {
    ...
    for (auto it = g->GetVertices(); it.first != it.second; ++it.first) {
      if (_v2v.find(*it.first) != _v2v.end())
        continue;
      AddVertex(*it.first);
    }
  ...
  void IXAssignmentGraph::PreassignElectrons() {
    for (AGVertex v : _v) {
      ...
      if (v->IsEdgeMapped()) {
        ...
        preassign = __VertexPrePlace(
            Degree(v), v->GetSourceVertex()->GetAtom()->GetElement());
      }
