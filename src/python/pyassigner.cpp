...
void GeneratePyElectronAssigner(pybind11::module& m) {
  ...
  py::class_<IXElectronAssigner, ElectronAssigner> PyAssigner(m, "ElectronAssigner");
  ...
  py::class_<Settings, std::shared_ptr<Settings>> PySettings(PyAssigner, "Settings");
  ...
  py::class_<LOSetters, std::shared_ptr<LOSetters>>(PySettings, "LocalOptimisation")
