...
namespace indigox {
  ...
  void _NTDecomp::SetInput(TDecomp g, TDVertex root) {
    ...
    while (queue.size()) {
      ...
      for (; nbrs.first != nbrs.second; ++nbrs.first) {
        if (used.find(*nbrs.first) != used.end())
          continue;
        AddEdge(td2ntd.at(v), td2ntd.at(*nbrs.first));
        queue.push_back(*nbrs.first);
      }
      used.insert(v);
      if (td->Degree(v) == 1 && v != root)
        AddEdge(td2ntd.at(v), AddVertex());
    }
        ...
        while (InDegree(leaf)) {
          ...
          for (MolVertPair uv : in_edges) {
            ...
            if (leaf_prop->bag.find(uu) == leaf_prop->bag.end() &&
                leaf_prop->bag.find(vv) == leaf_prop->bag.end())
              continue;
    ...
    for (NTDVertex parent : existing_vertices) {
      ...
      for (NTDVertex child : nbrs) {
        ...
        std::set_intersection(
            p->bag.begin(), p->bag.end(), c->bag.begin(), c->bag.end(),
            std::inserter(intersection, intersection.begin()));
        if (intersection == p->bag || intersection == c->bag)
          continue;
        NTDVertProp n;
        n.id = vert_count_++;
      ...
      for (NTDVertex child : nbrs) {
        ...
        if (intersection == c->bag) {
          while (p->bag.size() > c->bag.size() + 1) {
            // Introduce nodes
            difference.clear();
            std::set_difference(p->bag.begin(), p->bag.end(), c->bag.begin(),
                                c->bag.end(),
                                std::inserter(difference, difference.begin()));
            //            size_t occurCount = 10000000;
            MolVertPair intro = difference.back();
            //            for (MolVertPair v : difference) {
            //              size_t currentCount = std::count(locs.begin(),
            //              locs.end(), v); if (currentCount < occurCount) {
          ...
          while (p->bag.size() < c->bag.size() - 1) {
            difference.clear();
            std::set_difference(c->bag.begin(), c->bag.end(), p->bag.begin(),
                                p->bag.end(),
                                std::inserter(difference, difference.begin()));
            ...
            if (forget.first == nullptr)
              forget = difference.back();
    ...
    for (NTDVertex v : existing_vertices) {
      ...
      if (InDegree(v) == 0) {
        ...
        p->kind = std::make_pair('L', nullpair);
      else if (Degree(v) == 2)
        p->kind = std::make_pair('J', nullpair);
      else {
        NTDVertProp *c = GetProperties(*(GetNeighbours(v).first));
  ...
  //    boost::associative_property_map<std::map<NTDVertex, int>>
  //    indexMap(idxMap); NTDVertIterPair verts = GetVertices(); for (int i = 0;
  //    verts.first != verts.second; ++verts.first, ++i) {
