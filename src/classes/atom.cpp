...
//  : mol(CreateMolecule()), atm(mol), fftype(CreateGenericTestFFAtom().imp) { }
  ...
  void Atom::save(Archive &archive, const uint32_t) const {
  ...
  template <typename Archive>
  void Atom::load_and_construct(Archive &archive,
  ...
  INDIGOX_SERIALISE_CONSTRUCT(Atom);
 ...
 */
  ...
  Atom::Atom(Molecule& m)
  : utils::IXCountableObject<Atom>(), _mol(m.weak_from_this()),
  _elem(GetPeriodicTable().GetUndefined()), _fc(0), _tag(0), _implicitH(0),
  _name(), _pos(0.0,0.0,0.0), _partial(0.0), _stereo(Stereo::UNDEFINED),
  _aromatic(false) { }
 ...
 */
  ...
  void Atom::SetElement(const Element& e) {
    if (e != _elem) {
      _elem = e;
      _mol.lock()->ModificationMade();
    }
  }
 ...
 */
  ...
  void Atom::AddBond(Bond &b) {
    _bnds.emplace_back(b.weak_from_this()); }
  void Atom::AddAngle(Angle &a) {
    _angs.emplace_back(a.weak_from_this()); }
  void Atom::AddDihedral(Dihedral &d) {
    _dhds.emplace_back(d.weak_from_this()); }
  
  void Atom::RemoveBond(Bond &b) {
    auto pos = utils::WeakContainsShared(_bnds.begin(), _bnds.end(),
                                         b.shared_from_this());
    if (pos != _bnds.end()) _bnds.erase(pos);
  }
  
  void Atom::RemoveAngle(Angle &a) {
    auto pos = utils::WeakContainsShared(_angs.begin(), _angs.end(),
                                         a.shared_from_this());
    if (pos != _angs.end()) _angs.erase(pos);
  }
  
  void Atom::RemoveDihedral(Dihedral &d) {
    auto pos = utils::WeakContainsShared(_dhds.begin(), _dhds.end(),
                                         d.shared_from_this());
    if (pos != _dhds.end()) _dhds.erase(pos);
  }
 ...
 */
  ...
  std::string Atom::ToString() {
    std::stringstream ss;
    ss << "Atom(" << GetIndex() << ", " << _elem.GetSymbol() << ")";
    return ss.str();
  }
