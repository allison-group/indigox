...
namespace indigox::algorithm {
  ...
  std::string __PrintVertexName(const graph::AGVertex &v) {
    ...
    names.reserve(2);
    names.emplace_back(
        v->GetSourceEdge()->GetBond()->GetSourceAtom()->GetName());
    names.emplace_back(
        v->GetSourceEdge()->GetBond()->GetTargetAtom()->GetName());
  ...
  SortOrder GetVertexSortOrder(const graph::AGVertex &v) {
    if (v->IsVertexMapped()) {
      ...
      case 7: // Nitrogen
        switch (a->NumBonds()) {
        case 1:
          return SortOrder::N_ONE;
        case 2:
          return SortOrder::N_TWO;
        case 3:
          return SortOrder::N_THREE;
        default:
      ...
      case 8: // Oxygen
        switch (a->NumBonds()) {
        case 1:
          return SortOrder::O_ONE;
        case 2:
          return SortOrder::O_TWO;
        default:
          return SortOrder::UNDEFINED;
        }
      case 9: // Fluorine
      ...
      case 15: // Phosphorus
        switch (a->NumBonds()) {
        case 1:
          return SortOrder::P_ONE;
        case 2:
          return SortOrder::P_TWO;
        case 3:
          return SortOrder::P_THREE;
        default:
          return SortOrder::UNDEFINED;
        }
      case 16: // Sulfur
  ...
  void IXAStarOptimisation::Run() {
    ...
    for (graph::AGVertex v : _locs)
      std::cout << __PrintVertexName(v) << std::endl;
    while (!_q.empty()) {
      ...
      if (source.unchange_mask.all()) {
        if (target_score == _limit)
          target_score = source.path;
        if (source.path == target_score) {
          _results.emplace_back(source.assignment);
          if (_max_results && _results.size() >= _max_results)
            break;
        } else if (source.Total() > target_score)
          break;
        continue;
      } else if (target_score != _limit && source.path > target_score)
        break;
      ...
      for (size_t i = 0; i < num_nbrs; ++i) {
        ...
        size_t nbr_index =
            source.nbr_begin_idx + _loc_counts[GetLocationPosition(nbr_vertex)];
        ...
        // Unchange_mask and calc_mask are going to be the same between
        // neighbours So should be able to move outside of loop for very minor
        // speed up
      ...
      if (_len_limit > 0 && _q.size() > _len_limit)
        break;
    }
    _q.clear();
    if (target_score != _limit)
      _min_score = target_score;
    else
      _min_score = _inf;
  }
  ...
  void IXAStarOptimisation::DetermineLocationCounts() {
    ...
    auto loc_order = [](const graph::AGVertex &a, const graph::AGVertex &b) {
      if (a->IsVertexMapped() && b->IsEdgeMapped())
        return true;
      else
        return false;
    };
  ...
  LocMask
  IXAStarOptimisation::DetermineCalculableLocations(const QueueItem &q) const {
  ...
  score_t IXAStarOptimisation::CalculateNewPathScore(const QueueItem &q) {
    if (_num_e < q.assignment.count())
      return _inf;
    if ((q.assignment.size() - q.nbr_begin_idx) <
        (_num_e - q.assignment.count()))
      return _inf;
  ...
  score_t
  IXAStarOptimisation::CalculateHeuristicScore(const QueueItem &q) const {
    if (q.assignment.count() == _num_e)
      return 0;
    if (q.assignment.count() > _num_e)
      return _inf;
  ...
  score_t IXAStarOptimisation::Promiscuous(const QueueItem &q) const {
    ...
    while (pos < uncalcuable.size()) {
      ...
      if (v->IsVertexMapped()) {
        key_t k1 =
            v->GetSourceVertex()->GetAtom()->GetElement().GetAtomicNumber();
        ...
        key_t k1 =
            u1->GetSourceVertex()->GetAtom()->GetElement().GetAtomicNumber();
        k1 += (u2->GetSourceVertex()->GetAtom()->GetElement().GetAtomicNumber()
               << 8);
  ...
  score_t IXAStarOptimisation::Abstemious(const QueueItem &q) const {
    ...
    for (size_t i = 0; i < _unique_locs.size(); ++i) {
      if (q.unchange_mask[i])
        extra_positions.emplace_back(0);
      else if (extras <= _loc_counts[i])
        extra_positions.emplace_back(extras);
      else
        extra_positions.emplace_back(_loc_counts[i]);
    }
  ...
  void IXAStarOptimisation::Initalise(const Molecule &mol) {
    ...
    //    for (graph::AGVertex v : _locs) std::cout << __PrintVertexName(v) <<
    //    std::endl;
    ...
    for (size_t i = 0; i < _unique_locs.size(); ++i) {
      std::cout << __PrintVertexName(_unique_locs[i]) << ": "
                << _req_unchange[i] << std::endl;
    }
