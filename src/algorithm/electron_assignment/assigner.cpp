...
namespace indigox::algorithm {
  ...
  eastl::vector_set<Element> settings::AllowedElements = {
      GetPeriodicTable().GetElement("H"), GetPeriodicTable().GetElement("C"),
      GetPeriodicTable().GetElement("N"), GetPeriodicTable().GetElement("O"),
      GetPeriodicTable().GetElement("S"), GetPeriodicTable().GetElement("P"),
      GetPeriodicTable().GetElement("F"), GetPeriodicTable().GetElement("Cl"),
  ...
  AAlgo::AssignAlgorithm(const ScoreTable &t)
      : _mol(), _g(), _locs(), _num_e(0), _opts(0), _inf(settings::Infinity),
        _min_score(_inf), _limit(_inf), _results(), _table(t),
        _max_charge(settings::MaxChargeMagnitude),
  ...
  AssignMask AAlgo::CalculateUpperLimit() {
    ...
    while (initial.count() < _num_e) {
      ...
      all_location.erase(
          std::remove(all_location.begin(), all_location.end(), min_nbr_pos));
    }
  ...
  void AAlgo::SetAssignment(const AssignMask &a) {
    ...
    while (i < changed.size()) {
      ...
      if (lost[i])
        v->SetAssignedCount(v->GetAssignedCount() - delta);
      else
        v->SetAssignedCount(v->GetAssignedCount() + delta);
  ...
  bool AAlgo::ApplyAssignment(size_t idx) {
    ...
    for (auto vs = _g->GetVertices(); vs.first != vs.second; ++vs.first) {
      ...
      if (v->IsEdgeMapped()) {
        ...
        case 2:
          b->SetOrder(BondOrder::SINGLE);
          break;
        case 4:
          b->SetOrder(BondOrder::DOUBLE);
          break;
        case 6:
          b->SetOrder(BondOrder::TRIPLE);
          break;
        case 8:
          b->SetOrder(BondOrder::QUADRUPLE);
          break;
        default:
          b->SetOrder(BondOrder::UNDEFINED);
          break;
        }
      } else {
  ...
  void IXElectronAssigner::LoadScoreTable() {
    ...
    for (std::string line : lines) {
      ...
      if (items.size() == 3) { // Atom score
        ...
        if (tmp_dat.find(tt) == tmp_dat.end())
          tmp_dat.emplace(tt);
        ...
        if (tmp_dat.find(t1) == tmp_dat.end())
          tmp_dat.emplace(t1);
        if (tmp_dat.find(t2) == tmp_dat.end())
          tmp_dat.emplace(t2);
        // Determine local/global minimums
    ...
    for (auto eets_quad : tmp_dat) {
      if (!(eets_quad.second.GetAtomicNumber())) {
        ...
        if (eets_quad.third < 0)
          k += 1 << 15;
        _table.emplace(k,
                       eets_quad.fourth - local_atom_min.at(eets_quad.first));
      } else {
        key_t k = eets_quad.first.GetAtomicNumber();
  ...
  size_t IXElectronAssigner::Run() {
    ...
    Molecule mol = _mol.lock();
    if (!mol)
      throw std::runtime_error("Molecule is invalid");
    ...
    static const op valid_lo_cache =
        op::All | op::Some | op::None | op::Default;
