...
namespace indigox::algorithm {
  ...
  void IXLocalOptimisation::Run() {
    ...
    do {
      ...
      if (_lo_opts[__all_mins])
        end = current_min_assigns.end();
      else
        end = ++current_min_assigns.begin();
    ...
    if (current_min_score != _inf) {
      for (AssignMask m : current_min_assigns) {
        ...
        if (_max_results > 0 && _results.size() >= _max_results)
          break;
      }
    }
  ...
  void IXLocalOptimisation::BuildLocationMasks() {
    ...
    for (size_t i = 0; i < _locs.size(); ++i) {
      ...
      if (current == previous)
        ...
        if (i)
          _loc_masks.emplace(previous, bitmask);
        bitmask.reset();
        bitmask.set(i);
        previous = current;
      }
  ...
  void
  IXLocalOptimisation::GetNeighbourAssignments(const AssignMask &m,
                                               std::vector<AssignMask> &nbrs) {
    nbrs.clear();
