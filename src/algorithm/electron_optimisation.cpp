...
void ElectronOpt::DetermineElectronsToAdd() {
  ...
  for (MolVertexIter v = vs.first; v != vs.second; ++v)
    ecount += molGraph_->GetProperties(*v)
                  ->atom->GetElement()
  ...
  if (opt_::USE_ELECTRON_PAIRS && electronsToAdd_ % 2)
    throw std::runtime_error(
        "Unable to handle odd number of electrons when using electron pairs.");
...
void ElectronOpt::DeterminePotentialElectronLocations() {
  ...
  for (MolVertexIter v = vs.first; v != vs.second; ++v) {
    ...
    if (molGraph_->Degree(*v) > 2)
      octet = (int8_t)molGraph_->GetProperties(*v)
                  ->atom->GetElement()
                  ->GetHypervalentOctet();
    else
      octet =
          (int8_t)molGraph_->GetProperties(*v)->atom->GetElement()->GetOctet();
  ...
  for (MolEdgeIter e = es.first; e != es.second; ++e) {
    ...
    if (molGraph_->Degree(u) > 2)
      u_oct = (int8_t)molGraph_->GetProperties(u)
                  ->atom->GetElement()
                  ->GetHypervalentOctet();
    else
      u_oct =
          (int8_t)molGraph_->GetProperties(u)->atom->GetElement()->GetOctet();
    if (molGraph_->Degree(v) > 2)
      v_oct = (int8_t)molGraph_->GetProperties(v)
                  ->atom->GetElement()
                  ->GetHypervalentOctet();
    else
      v_oct =
...
#define NUM_DECIMAL_PLACE 5
  for (size_t i = 2; i < at_items.size(); i += 3) {
    ...
    if (sep_pos == std::string::npos)
      throw std::invalid_argument(at_items[i] +
                                  std::string(" is not a decimal value."));
    lead = at_items[i].substr(0, sep_pos);
    tail = at_items[i].substr(sep_pos + 1, NUM_DECIMAL_PLACE);
    while (tail.size() < NUM_DECIMAL_PLACE)
      tail.append("0");
    int64_t score;
    try {
  ...
  for (size_t i = 3; i < bn_items.size(); i += 4) {
    ...
    if (sep_pos == std::string::npos)
      throw std::invalid_argument(bn_items[i] +
                                  std::string(" is not a decimal value."));
  ...
  for (size_t i = 0; i < at_items.size(); i += 3) {
    ...
    if (Z == 0)
      throw std::invalid_argument(
          at_items[i] + std::string(" is not a valid atomic symbol."));
    int fc;
    try {
      fc = std::stoi(at_items[i + 1]);
    } catch (const std::invalid_argument &e) {
      throw std::invalid_argument(at_items[i + 1] +
                                  std::string(" is not an integer value."));
    }
    uint32_t k = Z + (uint32_t)(std::abs(fc) << 8);
    if (fc < 0)
      k += (1 << 15);
    FCSCORE val =
        (FCSCORE)(std::stoll(at_items[i + 2]) - la_mins.at(at_items[i]));
  ...
  for (size_t i = 0; i < bn_items.size(); i += 4) {
    ...
    if (Zb == 0)
      throw std::invalid_argument(
          at_items[i + 1] + std::string(" is not a valid atomic symbol."));

    uint32_t order;
    try {
      order = (uint32_t)std::stoi(bn_items[i + 2]);
    } catch (const std::invalid_argument &e) {
      throw std::invalid_argument(bn_items[i + 2] +
                                  std::string(" is not an integer value."));
    }
    if (order < 1)
      throw std::invalid_argument(bn_items[i + 2] +
                                  std::string(" is an invalid bond order."));
    ...
    std::pair<std::string, std::string> bondType =
        std::make_pair(bn_items[i], bn_items[i + 1]);
  ...
  for (MolVertPair &vp : possibleLocations_) {
    ...
    if (vp.first == vp.second) {
      ...
      case 7: // Nitrogen
        ...
        case 1:
          p->sort_score = SortOrder::N_ONE;
          break;
        case 2:
          p->sort_score = SortOrder::N_TWO;
          break;
        case 3:
          p->sort_score = SortOrder::N_THREE;
          break;
        default:
          p->sort_score = SortOrder::UNDEFINED;
          break;
        }
        break;
      case 8: // Oxygen
        switch (molGraph_->Degree(vp.first)) {
        case 1:
          p->sort_score = SortOrder::O_ONE;
          break;
        case 2:
          p->sort_score = SortOrder::O_TWO;
          break;
        default:
          p->sort_score = SortOrder::UNDEFINED;
          break;
      ...
      case 15: // Phosphorus
        switch (molGraph_->Degree(vp.first)) {
        case 1:
          p->sort_score = SortOrder::P_ONE;
          break;
        case 2:
          p->sort_score = SortOrder::P_TWO;
          break;
        case 3:
          p->sort_score = SortOrder::P_THREE;
          break;
        default:
          p->sort_score = SortOrder::UNDEFINED;
          break;
