...
namespace indigox::algorithm {
  ...
  int64_t ConnectedComponents(BaseGraph<V,E,S,D,VP,EP>& G, Container& bits) {
  ...
  template int64_t ConnectedComponents(CondensedMolecularGraph::graph_type&,
  ...
  template <class BitSet, class V, class E, class S, class D, class VP, class EP>
  void __build_neighbours_bitsets(graph::BaseGraph<V,E,S,D,VP,EP>& G,
          const typename graph::BaseGraph<V,E,S,D,VP,EP>::VertContain& vertices,
  ...
  template <class BitSet, class V, class E, class S, class D, class VP, class EP>
  void __populate_subgraphs(BaseGraph<V,E,S,D,VP,EP>& G,
  ...
  int64_t ConnectedSubgraphs(BaseGraph<V,E,S,D,VP,EP>& G, std::vector<S>& subs,
                             size_t min, size_t max)
  {
    ...
    if (G.NumVertices() <= 32)  _runner<Bitset32>(G, subs, min, max);
    else if (G.NumVertices() <= 64) _runner<Bitset64>(G, subs, min, max);
    else if (G.NumVertices() <= 128) _runner<Bitset128>(G, subs, min, max);
    else _runner<BitsetMax>(G, subs, min, max);
    return subs.size();
  }
