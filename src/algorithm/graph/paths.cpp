...
namespace indigox::algorithm {
  ...
  template std::vector<MGEdge>
  ShortestPath(MolecularGraph::graph_type&, MGVertex, MGVertex);
  ...
  void AllSimplePaths(BaseGraph<V,E,S,D,VP,EP>& G, V source, V target,
    ...
    std::vector<V> visited; visited.reserve(G.NumVertices());
    visited.push_back(source);
    
    std::vector<NbrsType> stack; stack.reserve(G.NumVertices());
  ...
  template void
  AllSimplePaths(CondensedMolecularGraph::graph_type&, CMGVertex, CMGVertex,
                 std::vector<std::vector<CMGEdge>>&, int64_t);
  template void
